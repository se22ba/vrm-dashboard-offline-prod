 ...
 * 1) Busca la tabla de "Devices".
 * 2) Busca filas label/value en TODO el DOM (versión precisa).
 * 3) Busca sección alrededor de "Devices/Dispositivos".
...
export async function parseIndexMhtml(filePath) {
  ...
  // 3) sección alrededor de “Devices”
...
function readGlobalsFromDevicesTable(html) {
  ...
  const labelsMap = {
    ...
    'pérdida de señal': 'signalLoss',
    'perdida de señal': 'signalLoss',
  ...
  $('table').each((_, tbl) => {
    ...
    $(tbl).find('tr').each((__, tr) => {
      ...
      const rightText = $(tds[tds.length - 1]).text();
...
function readGlobalsAnywhere(html) {
  ...
  const map = [
    { re: /(total channels|total de canales|canais totais)/i, key: 'totalChannels' },
    { re: /(offline channels|canales offline|canais offline)/i, key: 'offlineChannels' },
    { re: /(active recordings|grabaciones activas|grava(?:coes|ções) ativas)/i, key: 'activeRecordings' },
    { re: /\b(idle|inactivo(?:s)?)\b/i, key: 'idle' },
    { re: /(signal loss|perdida de senal|p[eé]rdida de se[nñ]al|perda de sinal)/i, key: 'signalLoss' }
  ];

  const out = { totalChannels: 0, offlineChannels: 0, activeRecordings: 0, idle: 0, signalLoss: 0 };

  // 2.1) filas explícitas label/value o celdas contiguas
  $('.label, .leftLabel, .rightLabel').each((_, lab) => {
    const txt = norm($(lab).text());
    const hit = map.find(m => m.re.test(txt));
    if (!hit) return;

    // sólo el .value HERMANO (no global)
    let node = $(lab).nextAll('.value').first();

    // o la celda inmediata si es tabla
    if (!node.length) {
  ...
  // 2.2) si todavía no hay suficiente, buscamos en tablas (celda contigua)
  if (!someValue(out)) {
    $('table').each((_, tbl) => {
      $(tbl).find('tr').each((__, tr) => {
        const tds = $(tr).find('td,th');
...
function readGlobalsFromSection(sectionHtml) {
  ...
  const read = (labels) => {
    ...
    // texto plano como último intento en esta sección
  ...
  const L = {
    total:   '(Total\\s*channels|Total\\s*de\\s*canales|Canais\\s*totais)',
    offline: '(Offline\\s*channels|Canales\\s*offline|Canais\\s*offline)',
    active:  '(Active\\s*recordings|Grabaciones\\s*activas|Grava(?:ções|coes)\\s*ativas)',
    idle:    '(\\bIdle\\b|Inactivo(?:s)?)',
    signal:  '(Signal\\s*loss|P(?:é|e)rdida\\s*de\\s*se(?:ñ|n)al)'
  };
...
function readGlobalsFromPlain(text) {
  ...
  const L = {
    total:   '(Total\\s*channels|Total\\s*de\\s*canales|Canais\\s*totais)',
    offline: '(Offline\\s*channels|Canales\\s*offline|Canais\\*offline)',
    active:  '(Active\\s*recordings|Grabaciones\\s*activas|Grava(?:ções|coes)\\s*ativas)',
    idle:    '(\\bIdle\\b|Inactivo(?:s)?)',
    signal:  '(Signal\\s*loss|P(?:é|e)rdida\\s*de\\s*se(?:ñ|n)al)'
  };
...
function extractPerVrmByAnchors(html) {
  ...
  let m;
  while ((m = ipBackslash.exec(html)) !== null) {
    const key = `${m[1]}\\${m[2]}`;
    const slice = html.slice(Math.max(0, m.index), m.index + 20000);
    perVrm[key] = {
      totalChannels:    pickIntNear(slice, /(Total\s*channels|Total\s*de\s*canales|Canais\s*totais)/i),
      offlineChannels:  pickIntNear(slice, /(Offline\s*channels|Canales\s*offline|Canais\s*offline)/i),
      ...
      signalLoss:       pickIntNear(slice, /(Signal\s*loss|P(?:é|e)rdida\s*de\s*se(?:ñ|n)al)/i),
    };
  }
...
function extractPerVrmByDom(html) {
  ...
  const RX = {
    ...
    total: /Total\s*channels|Total\s*de\s*canales|Canais\s*totais/i,
    offline: /Offline\s*channels|Canales\s*offline|Canais\s*offline/i,
    active: /Active\s*recordings|Grabaciones\s*activas|Grava(ções|coes)\s*ativas/i,
    idle: /\bIdle\b|Inactivo(s)?/i,
    signal: /Signal\s*loss|P(é|e)rdida\s*de\s*se(ñ|n)al/i,
  };
  let current = null;
